...
type TTokenRepository interface {
	Get(id uint) (*TToken, error)
	Create(data *TToken) error
	Update(data *TToken) error
...
type TTokenRepositoryImpl struct {
	mutex sync.Mutex
	db    *gorm.DB
}

func NewTTokenRepository(db *gorm.DB) TTokenRepository {
	return &TTokenRepositoryImpl{
		db: db,
	}
}

func (s *TTokenRepositoryImpl) Get(id uint) (*TToken, error) {
	s.mutex.Lock()
...
func (s *TTokenRepositoryImpl) Create(entity *TToken) error {
	s.mutex.Lock()
	defer s.mutex.Unlock()
...
func (s *TTokenRepositoryImpl) Update(entity *TToken) error {
	s.mutex.Lock()
	defer s.mutex.Unlock()
...
func (s *TTokenRepositoryImpl) Delete(id uint) error {
	s.mutex.Lock()
	defer s.mutex.Unlock()
...
func (s *TTokenRepositoryImpl) GetPage(
	sortRequest []request.Sort,
	filterRequest []request.Filter,
	searchRequest string,
	pageInt int,
	...
	util.Log("INFO", "repository", "GetPageTToken", "")
	...
	page := response.Page{
		Content:          ToTTokenResponsesParallel(entities),
		Pageable:         pageable,
		Sort:             sort,
	...
	util.Log("INFO", "repository", "GetPageTToken", "sort is empty: "+strconv.FormatBool(sort.Empty))
