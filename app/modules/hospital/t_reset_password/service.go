...
type TResetPasswordService interface {
	...
	Create(payload *TResetPasswordRequest, mUserId uint) error
	Update(payload *TResetPasswordRequest, mUserId uint) error
...
type TResetPasswordServiceImpl struct {
	repo TResetPasswordRepository
}

func NewTResetPasswordService(repo TResetPasswordRepository) TResetPasswordService {
	return &TResetPasswordServiceImpl{
		repo: repo,
	}
}

func (s *TResetPasswordServiceImpl) Get(id uint) (*TResetPassword, error) {
...
func (s *TResetPasswordServiceImpl) Create(payload *TResetPasswordRequest, mUserId uint) error {

	data := ToTResetPasswordEntity(*payload, nil, mUserId)
...
func (s *TResetPasswordServiceImpl) Update(payload *TResetPasswordRequest, mUserId uint) error {
	if payload.Id == nil {
	...
	existing.OldPassword = payload.OldPassword
	existing.NewPassword = payload.NewPassword
	existing.ResetFor = payload.ResetFor
...
func (s *TResetPasswordServiceImpl) Delete(id uint) error {
	_, err := s.repo.Get(id)
	if err != nil {
		return err
	}
...
func (s *TResetPasswordServiceImpl) GetPage(
	sortRequest []request.Sort,
	filterRequest []request.Filter,
	searchRequest string,
	pageInt int,
