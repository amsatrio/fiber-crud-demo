...
type MMedicalItemSegmentation struct {
...
func (MMedicalItemSegmentation) TableName() string {
	return "m_medical_item_segmentation"
}

type MMedicalItemSegmentationRequest struct {
...
type MMedicalItemSegmentationResponse struct {
	Id       *uint   `form:"id" json:"id" xml:"id"`
	Name     *string `form:"name" json:"name" xml:"name" validate:"max=50"`
	IsDelete bool    `form:"isDelete" json:"isDelete" xml:"isDelete"`
}

func ToMMedicalItemSegmentationEntity(req MMedicalItemSegmentationRequest, imageData []byte, userId uint) MMedicalItemSegmentation {
...
func ToMMedicalItemSegmentationResponse(entity MMedicalItemSegmentation) MMedicalItemSegmentationResponse {
	res := MMedicalItemSegmentationResponse{
		Id:       &entity.Id,
		Name:     entity.Name,
...
func ToMMedicalItemSegmentationResponsesParallel(entities []MMedicalItemSegmentation) []MMedicalItemSegmentationResponse {
	numEntities := len(entities)
	responses := make([]MMedicalItemSegmentationResponse, numEntities)

	var wg sync.WaitGroup
	wg.Add(numEntities)

	for i, e := range entities {
		go func(index int, entity MMedicalItemSegmentation) {
			defer wg.Done()
			responses[index] = ToMMedicalItemSegmentationResponse(entity)
		}(i, e)
	}
