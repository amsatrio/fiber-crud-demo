...
type MMenuRoleRepository interface {
	Get(id uint) (*MMenuRole, error)
	Create(data *MMenuRole) error
	Update(data *MMenuRole) error
...
type MMenuRoleRepositoryImpl struct {
	mutex sync.Mutex
	db    *gorm.DB
}

func NewMMenuRoleRepository(db *gorm.DB) MMenuRoleRepository {
	return &MMenuRoleRepositoryImpl{
		db: db,
	}
}

func (s *MMenuRoleRepositoryImpl) Get(id uint) (*MMenuRole, error) {
	s.mutex.Lock()
...
func (s *MMenuRoleRepositoryImpl) Create(entity *MMenuRole) error {
	s.mutex.Lock()
	defer s.mutex.Unlock()
...
func (s *MMenuRoleRepositoryImpl) Update(entity *MMenuRole) error {
	s.mutex.Lock()
	defer s.mutex.Unlock()
...
func (s *MMenuRoleRepositoryImpl) Delete(id uint) error {
	s.mutex.Lock()
	defer s.mutex.Unlock()

	var entity MMenuRole
...
func (s *MMenuRoleRepositoryImpl) GetPage(
	sortRequest []request.Sort,
	filterRequest []request.Filter,
	searchRequest string,
	pageInt int,
	...
	util.Log("INFO", "repository", "GetPageMMenuRole", "")

	var entities []MMenuRole
	var entity MMenuRole
	...
	page := response.Page{
		Content:          ToMMenuRoleResponsesParallel(entities),
		Pageable:         pageable,
		Sort:             sort,
	...
	util.Log("INFO", "repository", "GetPageMMenuRole", "sort is empty: "+strconv.FormatBool(sort.Empty))
