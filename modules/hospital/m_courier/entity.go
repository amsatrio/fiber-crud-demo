...
func (MCourier) TableName() string {
    return "m_courier"
}

type MCourierRequest struct {
...
func ToMCourierResponse(entity MCourier) MCourierResponse {
    res := MCourierResponse{
        Id: &entity.Id,
        Name: entity.Name,
...
func ToMCourierResponsesParallel(entities []MCourier) []MCourierResponse {
    numEntities := len(entities)
    responses := make([]MCourierResponse, numEntities)

    var wg sync.WaitGroup
    wg.Add(numEntities)

    for i, e := range entities {
        go func(index int, entity MCourier) {
            defer wg.Done()
            responses[index] = ToMCourierResponse(entity)
        }(i, e)
    }
