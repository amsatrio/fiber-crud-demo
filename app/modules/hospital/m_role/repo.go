...
func (s *MRoleRepositoryImpl) Create(entity *MRole) error {
	s.mutex.Lock()
	defer s.mutex.Unlock()
...
func (s *MRoleRepositoryImpl) Update(entity *MRole) error {
	s.mutex.Lock()
	defer s.mutex.Unlock()
...
func (s *MRoleRepositoryImpl) GetPage(
	...
	util.Log("INFO", "repository", "GetPageMRole", "")
	...
	page := response.Page{
		Content:          ToMRoleResponsesParallel(entities),
		Pageable:         pageable,
		Sort:             sort,
	...
	util.Log("INFO", "repository", "GetPageMRole", "sort is empty: "+strconv.FormatBool(sort.Empty))
