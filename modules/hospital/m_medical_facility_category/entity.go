...
type MMedicalFacilityCategory struct {
...
func (MMedicalFacilityCategory) TableName() string {
    return "m_medical_facility_category"
}

type MMedicalFacilityCategoryRequest struct {
...
type MMedicalFacilityCategoryResponse struct {
    Id         *uint         `form:"id" json:"id" xml:"id"`
    Name       *string       `form:"name" json:"name" xml:"name" validate:"max=50"`
    IsDelete   bool          `form:"isDelete" json:"isDelete" xml:"isDelete"`
}
func ToMMedicalFacilityCategoryEntity(req MMedicalFacilityCategoryRequest, imageData []byte, userId uint) MMedicalFacilityCategory {
...
func ToMMedicalFacilityCategoryResponse(entity MMedicalFacilityCategory) MMedicalFacilityCategoryResponse {
    res := MMedicalFacilityCategoryResponse{
        Id: &entity.Id,
        Name: entity.Name,
...
func ToMMedicalFacilityCategoryResponsesParallel(entities []MMedicalFacilityCategory) []MMedicalFacilityCategoryResponse {
    numEntities := len(entities)
    responses := make([]MMedicalFacilityCategoryResponse, numEntities)

    var wg sync.WaitGroup
    wg.Add(numEntities)

    for i, e := range entities {
        go func(index int, entity MMedicalFacilityCategory) {
            defer wg.Done()
            responses[index] = ToMMedicalFacilityCategoryResponse(entity)
        }(i, e)
    }
