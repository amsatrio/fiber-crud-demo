...
func (MCustomerRelation) TableName() string {
    return "m_customer_relation"
}

type MCustomerRelationRequest struct {
...
type MCustomerRelationResponse struct {
    Id         *uint         `form:"id" json:"id" xml:"id"`
    Name       *string       `form:"name" json:"name" xml:"name" validate:"max=50"`
    IsDelete   bool          `form:"isDelete" json:"isDelete" xml:"isDelete"`
}
func ToMCustomerRelationEntity(req MCustomerRelationRequest, imageData []byte, userId uint) MCustomerRelation {
...
func ToMCustomerRelationResponse(entity MCustomerRelation) MCustomerRelationResponse {
    res := MCustomerRelationResponse{
        Id: &entity.Id,
        Name: entity.Name,
...
func ToMCustomerRelationResponsesParallel(entities []MCustomerRelation) []MCustomerRelationResponse {
    numEntities := len(entities)
    responses := make([]MCustomerRelationResponse, numEntities)

    var wg sync.WaitGroup
    wg.Add(numEntities)

    for i, e := range entities {
        go func(index int, entity MCustomerRelation) {
            defer wg.Done()
            responses[index] = ToMCustomerRelationResponse(entity)
        }(i, e)
    }
