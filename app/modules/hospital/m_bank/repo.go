...
type MBankRepository interface {
	Get(id uint) (*MBank, error)
	Create(data *MBank) error
	Update(data *MBank) error
...
type MBankRepositoryImpl struct {
	mutex sync.Mutex
	db    *gorm.DB
}

func NewMBankRepository(db *gorm.DB) MBankRepository {
	return &MBankRepositoryImpl{
		db: db,
	}
}

func (s *MBankRepositoryImpl) Get(id uint) (*MBank, error) {
	s.mutex.Lock()
...
func (s *MBankRepositoryImpl) Create(entity *MBank) error {
	s.mutex.Lock()
	defer s.mutex.Unlock()
...
func (s *MBankRepositoryImpl) Update(entity *MBank) error {
	s.mutex.Lock()
	defer s.mutex.Unlock()
...
func (s *MBankRepositoryImpl) Delete(id uint) error {
	s.mutex.Lock()
	defer s.mutex.Unlock()
...
func (s *MBankRepositoryImpl) GetPage(
	sortRequest []request.Sort,
	filterRequest []request.Filter,
	searchRequest string,
	pageInt int,
	...
	util.Log("INFO", "repository", "GetPageMBank", "")
	...
	page := response.Page{
		Content:          ToMBankResponsesParallel(entities),
		Pageable:         pageable,
		Sort:             sort,
	...
	util.Log("INFO", "repository", "GetPageMBank", "sort is empty: "+strconv.FormatBool(sort.Empty))
